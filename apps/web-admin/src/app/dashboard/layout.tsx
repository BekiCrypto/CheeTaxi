...
const NAV = [
  ...
  { href: '/dashboard/safety', label: 'Safety & SOS', icon: '🆘' },
    ...
    <div className="flex h-screen bg-ink-50">
      <aside className="hidden w-64 flex-col border-r border-ink-200 bg-white md:flex">
        <div className="flex h-16 items-center gap-2 border-b border-ink-100 px-6">
          ...
          <span className="font-bold text-ink-900">CheeTaxi</span>
        </div>
        ...
        <div className="border-t border-ink-100 p-3">
          <div className="mb-2 px-3 text-xs text-ink-500">
            Signed in as
            <div className="truncate font-semibold text-ink-800">{user.phone}</div>
            <div className="text-[10px] uppercase tracking-wider text-brand-600">{user.role}</div>
          </div>
          <button
            onClick={() => {
              clearTokens();
            ...
            className="w-full rounded-lg px-3 py-2 text-left text-sm font-medium text-ink-600 hover:bg-red-50 hover:text-red-700"
          >
            Sign out
          </button>
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-ink-200 bg-white/80 px-6 backdrop-blur md:hidden">
